...
import nc.bs.hzsb.sql.MidTableExcute;
...
public class MidTableFactory {
	...
	public AggSBNCBillHVO[] querySBBillVO(String datasource, String tableName,
			String id) throws Exception {
		AggSBNCBillHVO[] bills = null;
		SBNCBillBVO[] bvos = null;
		...
		String querySql = "select * from " + " hz_cw." + tableName
				+ " where reserve2 ='" + id + "'";
		...
		try {
			...
			bvos = (SBNCBillBVO[]) gp.selectBySql2(querySql,
					nc.vo.hzsb.sbvo.SBNCBillBVO.class);
			if (bvos.length < 1) {
	...
	public AggSBNCBillHVO[] agQuerySBBillVO(String datasource,
	 ...
	 */
	public int Insert2NCBills(AggSBNCBillHVO[] nCbillvo)
			throws BusinessException {
		...
		try {
			for (int i = 0; i < nCbillvo.length; i++) {
				SBNCBillBVO[] bvos = (SBNCBillBVO[]) nCbillvo[i]
						.getChildrenVO();
				for (SBNCBillBVO body : bvos) {
					String inssql = " insert into NCAD50_B  (sid, dataprimaryid, tablesource, datasource, issue, deptno, deptname,"
	...
	public void agRetVOIsReady(String tableName, AggSBNCBillHVO[] bills,
			Integer flag) {
		...
		for (int i = 0; i < bills.length; i++) {
			SBNCBillBVO[] bvos = (SBNCBillBVO[]) bills[i].getChildrenVO();
	...
	private AggSBNCBillHVO[] buildBills(SBNCBillBVO[] bvos) {

		List<AggSBNCBillHVO> lst = new ArrayList<AggSBNCBillHVO>();
		MapList<String, SBNCBillBVO> ml = new MapList<String, SBNCBillBVO>();
		for (SBNCBillBVO bvo : bvos) {
			String str = bvo.getReserve2();
			ml.put(str, bvo);
		}
		Iterator<String> it = ml.keySet().iterator();
		while (it.hasNext()) {
			SBNCBillHVO hvo = new SBNCBillHVO();
			String key = ((String) it.next());
			hvo.setVssid(key);
			List<SBNCBillBVO> bodys = ml.get(key);
			if (bodys != null) {
				AggSBNCBillHVO bill = new AggSBNCBillHVO();
				bill.setParentVO(hvo);
				bill.setChildrenVO(bodys.toArray(new SBNCBillBVO[0]));
				lst.add(bill);
			}
		}
		return lst.toArray(new AggSBNCBillHVO[0]); // 初始化空数组
	}
	...
	public void agRetVOIsDone(String tableName, AggSBNCBillHVO[] aggvos,
		...
		for (int i = 0; i < aggvos.length; i++) {
			SBNCBillBVO[] bvos = (SBNCBillBVO[]) aggvos[i].getChildrenVO();
			WaterID = bvos[0].getReserve2();
			if (i == 0)
				updateSql += WaterID;
			else if (i < 1000 * (k + 1) && i > 1000 * k) { // 实际大于1000 都会报SQL错误
	...
	public void dataRewriteError(String tableName_B, AggSBBillHVO[] vos,
		...
		for (AggSBBillHVO vo : vos) {
			...
			for (int i = 0; i < vo.getChildrenVO().length; i++) {
				...
				updateSql += ((SBNCBillBVO) vo.getChildrenVO()[i]).getSid();
			}
			updateSql += ")";
			try {
